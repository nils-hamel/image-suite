...
This tool was designed in the context of city digitization using a spherical camera. As a spherical camera capture full panoramic image, it is able to see everything surrounding it. With good weather, traveling in a street with such camera produces images seeing half buildings exposed to the sun while the other half are seen is their own shade. This tool was designed to correct this effect by normalizing the exposure helping subsequent process that use these image such as _structure from motion_.

## Usage example

Considering the case of the spherical camera, the following image gives an example of a capture in the streets of a city. As the camera see part of itself and the car on which it is attached, a mask is also considered for this example.
...
One can see how the local histogram correction allows to adapt the correction to the local exposure on the image. The building in the shade are more exposed while the sunny ones are a bit less.

If the color component are not linked (missing --linked/-l), the correction is applied component wise which destroy the colorimetry of the image but allows to maximize the correction as shown on this result :
...
As one can see, not linking the color components during correction can lead to more dangerous correction as the low exposed component can be scaled up increasing their noise. But this allows to reveal a bit more the shaded part of the image. This can be interesting for subsequent process such as _optical flow_ and _structure from motion_.

In order to get the desired correction, one as to play with the reduction factor _r_ and the kernel factor _k_. The smaller _r_ is, the greater _k_ has to be in order not to end up with long processing time. One has also to keep in mind that too large value of _r_ associated with small value of _k_ can lead to a correction of the feature of the image, as the correction become _too local_.

This tool can also be used to process other type of image such as old photography that shows exposure problem such as this aerial image of Paris in the 1920 years. As it comes with undesired features on the edges, a mask is also used for the correction of the image :
...
<br />
<i>Aerial image of Paris in the 1920 years - Image : IGN, France</i>
</p>
<br />

Using the following command :

    ./image-adaptative-histogram -s ign-photo-1920.jpg -m ign-photo-1920-mask.png -e ign-photo-1920-1.jpg -r 0.1 -k 0.065 -n 127.5 -d 64.0
...
One can see how the local exposure problem can be solved using the image processing methodology. In case of gray-scale images, one need to convert it into a _RGB_ image (gray channel repetition) before to submit it to the tool that only accept three-channel images.

As a last example, this tool can also be used to correct exposure artifacts due to the aging of photographic negative after their digitization. Using the following command :
